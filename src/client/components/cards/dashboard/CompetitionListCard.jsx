      ...
      <div className="competition-items">
        <div className="item">
          <div className="item__left">
            <h4>
              <Link to={`/super/competition/update/${n.id}`}>
                {n.title}
              </Link>
            </h4>
            <p className="text-muted" style={{ margin: 0 }}>
              <span>Dipost {epochToRelativeTime(n.created_at)}</span> oleh{" "}
              <Link title={n.author.username} to={`/dashboard/user/${n.author.username}`}>
                {n.author.username}
              </Link>,
              {n.created_at < n.updated_at ? ` update terakhir ${epochToRelativeTime(n.updated_at)}` : ' belum ada update'}
              <br />
              Kategori{" "}
